        ...
        <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                ...
                <nav class="navbar navbar-expand-lg bg-gradient-success shadow-success border-radius-lg pt-4 pb-3 d-flex justify-content-between">
                	<div>
                        <a type="button" href="<?= base_url() ?>tagihan" class="btn btn-sm btn-dark m-0 me-4">Back</a>
                    </div>
                      <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav mr-auto">
                          ...
                          <?php 
                            ...
                            <a type="button" class="btn btn-sm btn-info m-0 me-4" href="<?= base_url() ?>tagihan/detail_tahunan/<?= $detail->row()->id_santri;?>">Tahunan</a>
                        	<?php else : ?>
                            
                            <a type="button" class="btn btn-sm btn-info disabled m-0 me-4" href="<?= base_url() ?>tagihan/detail_tahunan/<?= $detail->row()->id_santri;?>">Tahunan</a> 
                          ...
                          <li class="nav-item">
                            ...
                            <a type="button" class="btn btn-sm btn-warning disabled m-0 me-4" href="<?= base_url() ?>tagihan/detail/<?= $detail->row()->id_santri;?>">Tambahan</a>
                            <?php else : ?>
            ...
            <div class="card-body px-0 pb-2">
                <div class="table-responsive p-0">
                    <table class="table align-items-center">
                        ...
                        <tbody>
                            ...
                            $total_tagihan = 0;
                            $total_bayar = $this->db->select('sum(gross_amount) as total_bayar')->where(['id_santri' => $detail->row()->id_santri, 'status_code' => 200])->get('tb_bayar')->row()->total_bayar;
                            $tb = $total_bayar;
                            foreach ($detail->result() as $key) :                                
                                ...
                                <tr class="<?php echo $lunas = ($key->tagihan == $bayar) ? 'text-success' : ''; ?>">
                ...
                <div class="row mx-4">
                   ...
                   <!--  <?php 
                        $total_bayar_bulanan = $this->db->select('sum(gross_amount) as total_bayar')->where(['id_santri' => $detail->row()->id_santri, 'status_code' => 200])->get('tb_bayar_bulanan')->row()->total_bayar;
                        $total_bayar_tahunan = $this->db->select('sum(gross_amount) as total_bayar')->where(['id_santri' => $detail->row()->id_santri, 'status_code' => 200])->get('tb_bayar_tahunan')->row()->total_bayar;
                        $total_tagihan_bulanan = $this->db->select('sum(tagihan_bulanan) as total_tagihan')->where(['id_santri' => $detail->row()->id_santri])->get('tb_tagihan_bulanan')->row()->total_tagihan;
                        $total_tagihan_tahunan = $this->db->select('sum(tagihan_tahunan) as total_tagihan')->where(['id_santri' => $detail->row()->id_santri])->get('tb_tagihan_tahunan')->row()->total_tagihan;
                    ...
                    <div class="col-md-6">
                        <form action="<?= base_url() ?>tagihan/bayar_detail/<?= $detail->row()->id_santri ?>" method="POST" id="form_bayar">
                            <div class="input-group input-group-outline mb-3">
                                <label for="bayar" class="form-label">Bayar</label>
                                <input type="number" class="form-control" name="bayar" id="bayar">
                            </div>
                            <button type="submit"  id="button-addon2" class="btn btn-outline-secondary btn-info">bayar</button>
                        </form>
                    </div>
                    <!-- <div class="col-md-4">
                        <table class="table ">
                            ...
                            <tr>
                                <th>Total Tagihan Bulanan</th>
                                <th> = </th>
                                <th> Rp <?= number_format($total_tagihan_bulanan, 0, ',', '.') ?></th>
                            </tr>
                            <tr>
                                <th>Total Bayar Bulanan </th>
                                <th> = </th>
                                <th> Rp <?= number_format($total_bayar_bulanan, 0, ',', '.')  ?></th>
                            </tr>
                            <tr>
                                <th>Total Tunggakan Bulanan </th>
                                <th> = </th>
                                <th> Rp <?= number_format($total_tagihan_bulanan - $total_bayar_bulanan, 0, ',', '.') ?></th>
                            </tr>
                        </table>
                    </div>
                    <div class="col-md-4">
                        <table class="table ">
                            ...
                            <tr>
                                <th>Total Tagihan Tahunan </th>
                                <th> = </th>
                                <th> Rp <?= number_format($total_tagihan_tahunan, 0, ',', '.') ?></th>
                            </tr>
                            <tr>
                                <th>Total Bayar Tahunan </th>
                                <th> = </th>
                                <th> Rp <?= number_format($total_bayar_tahunan, 0, ',', '.')  ?></th>
                            </tr>
                            <tr>
                                <th>Total Tunggakan Tahunan </th>
                                <th> = </th>
                                <th> Rp <?= number_format($total_tagihan_tahunan - $total_bayar_tahunan, 0, ',', '.') ?></th>
                            </tr>
                        </table>
                    </div>
                     -->
                            ...
                            <tbody>
                                ...
                                foreach ($pembayar->result() as $key) : ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#" onclick="hapus('<?= base_url() ?>tagihan/hapus_bayar/<?= $key->id_santri ?>?order_id=<?= base64_encode($key->order_id) ?>', 'Apakah Anda Ingin Menghapus ..?')"><i class="fa fa-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
...
<script>
    function edit(order_id, id_santri, total) {
        ...
        $('#form_edit_bayar').attr('action', '<?= base_url() ?>tagihan/update_bayar/' + id_santri);
        $('#editbayar').val(total);
        $('#order_id').val(order_id);
    }
    function bulanan(id) {
      $('#bulanan').attr('href', '<?= base_url() ?>tagihan/detail_bulanan/'  + id);
</script>