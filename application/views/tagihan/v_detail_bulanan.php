        ...
        <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                <!-- <div class="bg-gradient-success shadow-success border-radius-lg pt-4 pb-3 d-flex justify-content-between">
                 ...
                 <nav class="navbar navbar-expand-lg bg-gradient-success shadow-success border-radius-lg pt-4 pb-3 d-flex justify-content-between">
                    <div>
                        <a type="button" href="<?= base_url() ?>tagihan/bulanan" class="btn btn-sm btn-dark m-0 me-4">Back</a>
                    </div>
                      <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav mr-auto">
                          ...
                          <li class="nav-item">
                            ...
                            <a type="button" class="btn btn-sm btn-warning disabled m-0 me-4" href="<?= base_url() ?>tagihan/detail_bulanan/<?= $detail->row()->id_santri;?>">Bulanan</a>
                            <?php else : ?>
            ...
            <div class="card-body px-0 pb-2">
                <div class="table-responsive p-0">
                    <table class="table align-items-center">
                        ...
                        <tbody>
                            ...
                            $total_tagihan = 0;
                            $total_bayar = $this->db->select('sum(gross_amount) as total_bayar')->where(['id_santri' => $detail->row()->id_santri, 'status_code' => 200])->get('tb_bayar_bulanan')->row()->total_bayar;
                            $tb = $total_bayar;
                            foreach ($detail->result() as $key) :
                                $total_tagihan += $key->tagihan_bulanan;
                                if ($tb >= $key->tagihan_bulanan) {
                                    $bayar = $key->tagihan_bulanan;
                                } else if ($tb <= 0) {
                                    $bayar = 0;
                                } else {
                                    $bayar = $tb;
                                } ?>
                                <tr class="<?php echo $lunas = ($key->tagihan_bulanan == $bayar) ? 'text-success' : ''; ?>">
                                    ...
                                    <td>
                                        <?= "Rp " . number_format($key->tagihan_bulanan, 0, ',', '.'); ?>
                                    </td>
                                    <td>
                                        <?= "Rp " . number_format($bayar, 0, ',', '.'); ?>
                                    </td>
                                    <td>
                                        <?= "Rp " . number_format($key->tagihan_bulanan - $bayar, 0, ',', '.'); ?>
                                    </td>
                                    <!-- <td>
                ...
                <div class="row mx-4">
                    ...
                    <div class="col-md-6">
                        <form action="<?= base_url() ?>tagihan/bayar_detail_bulanan/<?= $detail->row()->id_santri ?>" method="POST" id="form_bayar">
                            ...
                            <tbody>
                                ...
                                foreach ($pembayar->result() as $key) : ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#" onclick="hapus('<?= base_url() ?>tagihan/hapus_bayar_bulanan/<?= $key->id_santri ?>?order_id=<?= base64_encode($key->order_id) ?>', 'Apakah Anda Ingin Menghapus ..?')"><i class="fa fa-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
...
<script>
    function edit(order_id, id_santri, total) {
        ...
        $('#form_edit_bayar').attr('action', '<?= base_url() ?>tagihan/update_bayar_bulanan/' + id_santri);
        $('#editbayar').val(total);
